 ...
 * {@link IOException}
 * , which is a subclass
 * of IOException.  A IOException will be thrown when
        ...
        implements Configurable
{
   ...
   * @throws IOException if the server reply does not match  (n,n,n,n),(n),(n)
   * @since 3.1
   */
  ...
  private boolean __storeFile(FTPCmd command, String remote, InputStream local)
          throws IOException
  {
    return _storeFile(command.getCommand(), remote, local, null);
  }

  private boolean __storeFile(FTPCmd command, String remote, InputStream local, TrackUploadProgressBar track)
          throws IOException
  {
    return _storeFile(command.getCommand(), remote, local, track);
  }
          ...
          throws IOException
  {
    ...
    try
    {
      if (track == null)
      {
        Util.copyStream(local, output, getBufferSize(), null, false);
        track.transferComplete();
      }else
      {
          Util.copyStream(local, output, getBufferSize(), track, false);
      }
          ...
          throws IOException
  {
    ...
    try
    {
      Util.copyStream(input, local, getBufferSize(), null, false);
  ...
  public boolean storeFile(String remote, InputStream local, TrackUploadProgressBar track)
          throws IOException
  {
    return __storeFile(FTPCmd.STOR, remote, local, track);
  }
  ...
  public FTPFile mlistFile(String pathname) throws IOException
  {
    ...
    if (success)
    {
      ...
      if (reply.length() < 3)
      {
        throw new IOException("Invalid server reply (MLST): '" + reply + "'");
      }
   ...
   *                                                                         from the server.
   * @throws org.apache.commons.net.ftp.parser.ParserInitializationException Thrown if the parserKey parameter cannot be
   ...
   * @throws IOException                                                     If an I/O error occurs while either sending a
   *                                                                         command to the server or receiving a reply from the server.
   * @throws org.apache.commons.net.ftp.parser.ParserInitializationException Thrown if the autodetect mechanism cannot
    ...
    if (__entryParser == null || (parserKey != null && !__entryParserKey.equals(parserKey)))
    {
      ...
      } else
      {
        ...
        } else
        {
          ...
          { // system type must have been empty above
